...
export const metadata: Metadata = {
  title: 'Suprathermal Ions (Prototype) | B. L. Alterman',
  description: 'Remnants of past energetic events - prototype page with rich summaries',
};

function loadTopicData(): ResearchTopicData {
  const filePath = path.join(process.cwd(), 'public/data/research-topics/suprathermal-ions.json');
  const fileContents = fs.readFileSync(filePath, 'utf8');
  return JSON.parse(fileContents);
}

export default function SuprathermalIonsPrototype() {
    ...
    <main className="flex-1 container mx-auto py-16 md:py-24 max-w-4xl">
      <div className="mb-8 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
        <p className="text-sm text-yellow-800 dark:text-yellow-200">
          ...
          <a href="/research/suprathermal-ions" className="ml-2 underline">
            View current page →
          </a>
        </p>
      </div>
