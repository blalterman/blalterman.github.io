...
export const metadata: Metadata = {
  title: 'Alfven Waves (Prototype) | B. L. Alterman',
  description: 'How magnetic waves shape solar wind speeds - prototype page with rich summaries',
};

function loadTopicData(): ResearchTopicData {
  const filePath = path.join(process.cwd(), 'public/data/research-topics/alfven-waves.json');
  const fileContents = fs.readFileSync(filePath, 'utf8');
  return JSON.parse(fileContents);
}

export default function AlfvenWavesPrototype() {
    ...
    <main className="flex-1 container mx-auto py-16 md:py-24 max-w-4xl">
      <div className="mb-8 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
        <p className="text-sm text-yellow-800 dark:text-yellow-200">
          ...
          <a href="/research/turbulence" className="ml-2 underline">
            View current page →
          </a>
        </p>
      </div>
